            ...
            <div class="form-group" >
                ...
                <select class="form-control" name="subject_part_id" required>
                    ...
                    @forelse($subject_parts as $subject_part)
                        @if($subjectQuestion)
                        <option value="{{ $subject_part->id }}"
                                {{ $subject_part->id==$subjectQuestion->subject_part_id?'selected':'' }}>
                            {{ $subject_part->name }}</option>
                        @else
...
<div class="col-md-4">
    <div class="col-md-12">
        <div class="form-group" >
            ...
            <input type="number" value="{{ isset($subjectQuestion)?$subjectQuestion->mark:old('mark') }}" name="mark" step="0.01" class="form-control border border-info" placeholder="Enter mark" required >
        </div>
    </div>
</div>
